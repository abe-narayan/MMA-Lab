...
await page.locator('.ms-mode', { hasText: 'Teams' }).click(); await sleep(300);
await page.selectOption('#ms-preset', '2v2');
...
const state = () => page.evaluate(() => ({ t: Math.round(performance.now()), btn: document.querySelector('.ms-head .btn--play')?.textContent, dis: document.querySelector('.ms-head .btn--play')?.disabled, prog: document.querySelector('.ms-progress')?.textContent?.slice(-40) ?? null, cancel: !!document.querySelector('.ms-head .page-actions button:not(.btn--play)'), tab: document.querySelector('.nav-item[aria-current=page]')?.id }));
for (const how of (process.argv[2] ? [process.argv[2]] : ['click', 'dblclick'])) {
  ...
  const s = [];
  if (how !== 'dblclick') await page.getByRole('button', { name: 'Run bout' }).click(); else await page.getByRole('button', { name: 'Run bout' }).dblclick();
